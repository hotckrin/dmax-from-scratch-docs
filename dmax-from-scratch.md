...
さて、制約条件(3A)は実際のゲームの条件に比べて、**より大きいレベルのスロットに装着できない** という点が厳しすぎました。
...
簡単のために一旦この非対称性を取っ払って考えると以下のような条件になります。

> 制約条件(3B): 装飾品はどのレベルのスロットにも装着できる

この場合、Lv1装飾品はLv2スロットに装着できるだけでなく、Lv2装飾品もLv1スロットに装着できるようになります。
...
以下の手順で実行できます。必須スキルを変更してみて動作を確認してみて下さい。
...
ダメージ計算式は概ね以下のような形になります。(こちらは推定された形式です。したがって、できるだけゲーム内の計算式に近づけてはありますが、ゲーム内の計算式と一致しない可能性があります。)

DMAX MHWilds で利用している完全な形のダメージ計算式が知りたい場合は `dmax.py` を参照してください。

```
{ (武器倍率) * (第1乗算スキル) + (加算スキル) } * (第2乗算スキル) * (溜め補正) * (物理肉質) * (ビン補正) * (モーション値) * (会心率補正)
```
...
例えば、巧撃Lv1が発動したときは `(巧撃Lv1が発動)=1` であり、制約条件(4) によって Lv2 以降は全て0になるので
...
この定義によって `Lv〇 の場合は～` のように条件分岐を書く必要がなくなります。
言い換えると 0, 1 のビット、真偽値をスキルレベルごとに用意することによってスキルレベルによる効果の条件分岐を実現しています。
...
`ヌシの魂` のような複数装備することによって発動するグループスキルも表現できている点にも注目してください。
スキルレベルをビット表現することによってグループスキルやシリーズスキルの効果も表現できます。
...
`巧撃` スキルの場合はLv1からLv5までを全部発動させて `(10 + 15 + 20 + 25 + 30) = 100` とするのが最適です。
...
`damage_skill_set` つまりダメージ計算式に寄与するスキルの全てについて、制約条件式(4) `const_skill_level_total` を定義しています。
...
ただし、基本的にスキルレベルは高いほうがより高いダメージが得られるため、このままでは全スキルで最大レベルが発動するのが最適となってしまいます。
しかし、実際のゲーム内では発動するスキルは装備している装備に付いているスキルのみ、という条件があるため、この制約を追加してやる必要があります。
つまり制約条件(5)が必要ですね。

> 制約条件(5): ダメージ計算式において有効なスキルは装備中のスキルのみ

スキル `巧撃` を例に取って考えます。
スキル `巧撃` はできるだけ高いレベルを発動したいですが、発動できるスキルレベルは装備している装備に付いている `巧撃` スキルのレベルの合計 によって上から抑えられます。
...
ただし会心率の上限は100%なのでまとめると `min(100, 会心率)` です。
...
会心率についても同様に補助変数を導入して立式を簡単にしましょう。<br>
...
発動率は `mdl.afr` に表されるため、目的関数は以下のようになります。
...
以上で `dmax-mini-4.py` の実装が完了し、最適化シミュに必要な制約条件を実装できました。
...
それでは `dmax-mini4.py` を実行し、出力された問題ファイルを SCIP ソルバーで最適化してみましょう。
