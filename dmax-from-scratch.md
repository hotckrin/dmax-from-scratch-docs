...
最適化シミュを実装するために、まずは「解の候補」と「最適解」とはなにかを考えましょう。
...
処理しやすくなる理由は条件分岐が消えるからです。防具数は膨大なので、最適化シミュを実装する際に、「ラギアヘルムαを装備しなかった場合～」などと if 文による条件分岐処理を書いていてはキリがありません。「ラギアヘルムαを装備しなかった場合」はラギアヘルムαの装備個数0個と表現することにより、すべてのパターンを1つの組み合わせで表現できます。
...
解1 の場合頭防具の合計数は 1 であり、モンハンワイルズの制約条件「頭防具を1個以下しか装備できない」を満たしていることがわかります。

![](images/table5.drawio.png)

同様にして各装備が持っている装備タイプや付属スキルなどの属性を新しい列として追加すると以下のようなテーブルになります。

弱点特効スキルがついている装備「レダゼルトヘルムγ」や「痛撃珠【３】」には「弱点特効」列に 1 が入力されています。
また、装備「反攻の護石Ⅲ」は巧撃スキルLv3が発動するので「巧撃」列に 3 が入力されています。
...
- 「頭・胴・腕・腰・脚・護石・武器」はそれぞれ1個以下しか装備できない
- 「弱点特効」スキルは最低でもLv3以上必要
