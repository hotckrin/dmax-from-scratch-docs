...
| 制約条件        | 制約内容                                                                                         |
|-----------------|--------------------------------------------------------------------------------------------------|
| 制約条件(1)     | 各部位の装備は1つ以下しか装備できない (部位は次の7つ 頭・胴・腕・腰・脚・護石・武器)             |
| 制約条件(2)     | ユーザが指定したスキルレベルのルールを満たす                                                     |
| 制約条件(3)     | 同一スキルにおいては Lv0 ～ Lv7 のどれか1つの効果のみがダメージ計算式において有効                |
| 制約条件(4)     | 装備している装備のスキルの効果のみがダメージ計算式において有効                                   |
| 制約条件(5)     | 会心率の上限は100%                                                                               |
| 制約条件(6)     | 装飾品はスロットレベル以上の大きさのスロットにしか装着できない                                   |

普通にゲームをしていると特に意識することはありませんが、モンハンワイルズには上記のように6個の制約条件があります。
ダメージ計算式に加えて上記6つの制約条件を全て実装すると最適化シミュが完成します！
...
さて、解1は解候補の1つでした。私たちが知りたいのは最適解なので、解の部分を変数 `q[j]` で表現することにします。
さらに、属性部分のテーブルも2次元配列 `p[i][j]` で表現してみましょう。

テーブルは以下のようになります。

i は属性を表しています。 `i = 頭, 胴, 腰` です。<br>
j は装備名を表しています。 `j = レダゼルトヘルムγ, レダゼルトメイルγ, ... 痛撃珠【３】` です。

![](images/table8.drawio.png)

2次元配列のパラメータ `p[i][j]` と解の変数 `q[j]` を利用すると制約式 (1) は以下のように書けます。
